  ...
  [t.SET_ENTITIES]: (state, {
    ...
    if (!isNil(result)) {
      if (isPlainObject(result)) {
        ...
        const entitiesType = Object.keys(result).find(enType => enType === type)

        if (isNil(entitiesType) || !result[entitiesType][0]) {
          return state
        }

        const entities = result[entitiesType]
    ...
    if (!isNil(entities) && !isEmpty(entities)) {
      const entitiesType = Object.keys(entities).find(enType => enType === type)

      if (!isNil(entitiesType) && Object.keys(entities[entitiesType])[0]) {
        const entitiesIds = Object.keys(entities[entitiesType])
